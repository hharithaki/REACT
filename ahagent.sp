...
#objects = #appliances + #graspable + {bench}.
#step = 0..n.
#boolean = {true, false}.
...
% prevent agent from starting unnecessary actions that will prolong the game time
holds(heated(P),I+1) :- holds(agent_hand(human,P),I).
holds(cooked(C),I+1) :- holds(agent_hand(human,C),I). 
holds(on(F,kitchentable),I+1) :- holds(agent_hand(human,F),I).
...
% prevent the agent from trying to execute an action on an object that is already in the hand of the other agent.
-occurs(find(R,O),I):- holds(agent_hand(T,O),I), #other_agents(T).
...
% impossible to grab something if another object is already in the hand of the agent.
-occurs(grab(R,O1),I):- holds(in_hand(R,O2),I), O1 != O2.

% impossible to put an object down if the surface location is not found.
-occurs(put(R,O,S),I) :- not holds(found(R,S),I), S != kitchentable.
